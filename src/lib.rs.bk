...
impl fmt::Display for Profiler{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            Profiler::PerfStat {task_clock, context_switches,
                                cpu_migrations, page_faults, cycles,
                                instructions, branches, branch_misses,
                                l1_dcache_loads, l1_dcache_load_misses,
                                llc_loads, llc_load_misses, seconds} =>
                {
                    write!(f,
            ...
            Profiler::PerfStat{..} =>
                    {
                        ...
                        z.retain(|&x| !x.contains("<not supported>")& !x.contains("panicked") &
